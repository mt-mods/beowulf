...
local LOG_BLOCKED = "blocked new account name %s from %s %s"
...
local function inform_admin(msg)
		...
		for ip_pattern, options in pairs(blacklist_ip) do
			if match_values(ip, ip_pattern, options) then
				...
				if has_beerchat then
					...
					beerchat.send_on_channel("*Beowulf*", beerchat.moderator_channel_name, msg)
				end
				return MSG_BLOCKED
			end
		end
...
if minetest.get_modpath("geoip") then
	...
	-- Register prejoin handler for more efficient blocking after first failure from blocked network
	...
	-- Register join handler to prevent creating new accounts from blocked network
...
minetest.register_chatcommand("block", {
	...
	func = function(name, param)
		...
		elseif param:find("^[%w_]+$") then
			...
			if has_beerchat and beerchat.jail then
				-- Any chat_jail errors will be ignored
				beerchat.jail.chat_jail(name , param)
			end
		else
			return false
		end
		...
		minetest.log("action", "[beowulf] "..msg)
		minetest.chat_send_player(name, msg..". Kick/ban/jail players if needed.")
	end
})
